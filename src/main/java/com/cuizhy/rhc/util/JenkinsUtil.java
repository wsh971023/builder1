...
@Slf4j
@Component
@Scope("prototype") // 声明为原型作用域,每次都会新建
public class JenkinsUtil {

    private static final String LOGIN_PATH = "/login";
    private static final String LOGIN_CHECK_PATH = "/j_spring_security_check";
    private static final String CRUMB_PATH = "/crumbIssuer/api/json";
    private static final String BUILD_PATH = "/job/{job_name}/build";
    ...
    // --- 每个实例独有的状态字段 ---
    private String jenkins_local_base_url;
    private String jenkins_local_username;
    private String jenkins_local_base_password;
    ...
    public void loginToJenkins(String url, String username, String password){
        ...
        try {
            log.info("正在请求登录页面...");
            HttpRequest loginPageRequest = requestManager.createRequestBuilder(url + LOGIN_PATH).GET().build();
            ...
            HttpRequest loginRequest = requestManager.createRequestBuilder(url + LOGIN_CHECK_PATH)
            ...
            log.info("登录成功...");
            jenkins_local_base_url = url;
            jenkins_local_username = username;
            jenkins_local_base_password = password;
        }catch (Exception e){
    ...
    public String getJenkinsCrumb(String url) {
        try{
            ...
            HttpRequest crumbRequest = requestManager.createRequestBuilder(url + CRUMB_PATH).GET().build();
            HttpResponse<String> crumbResponse = executeRequestWithRetry(crumbRequest, HttpResponse.BodyHandlers.ofString());
    ...
    public void triggerBuild(String url, String job_name){
        ...
        try {
            ...
            HttpResponse<String> buildResponse = executeRequestWithRetry(buildRequest, HttpResponse.BodyHandlers.ofString());
    ...
    public int getBuildNumber(String jenkins_url, String job_info_url,String job_name, boolean buildRequired) throws Exception {
        if (buildRequired) {
        ...
        HttpRequest jobInfoRequest = HttpRequest.newBuilder()
                .uri(URI.create(jenkins_url + job_info_url.replace("{job_name}", job_name)))
                .GET()
                .build();

        HttpResponse<String> jobInfoResponse = executeRequestWithRetry(jobInfoRequest, HttpResponse.BodyHandlers.ofString());
     ...
     * @throws Exception
     */
    public boolean getBuildStatus(String jenkins_url,String job_info_url,String build_status_url, String jobName) throws Exception {
        int buildNumber = getBuildNumber(jenkins_url, job_info_url, jobName, false);
        String statusUrl = String.format(build_status_url, buildNumber, jobName);

        HttpRequest statusRequest = HttpRequest.newBuilder()
                .uri(URI.create(jenkins_url + statusUrl.replace("{job_name}", jobName).replace("{build_number}", String.valueOf(buildNumber))))
                .GET()
                .header("Jenkins-Crumb", requestManager.getCrumb())
                .build();

        HttpResponse<String> statusResponse = executeRequestWithRetry(statusRequest, HttpResponse.BodyHandlers.ofString());
    ...
    public void downloadFile(String url,String filePath, String saveDir,String job_name) {
        log.info("准备下载文件: " + filePath + "...");

        try{
            String fileUrl = url + "/job/"+ job_name + "/ws/" + filePath;
            ...
            HttpResponse<InputStream> fileResponse = executeRequestWithRetry(fileRequest, HttpResponse.BodyHandlers.ofInputStream());
    ...
    public String getDownloadPath(String file_path){
    ...
    @SneakyThrows
    private <T> HttpResponse<T> executeRequestWithRetry(HttpRequest request, HttpResponse.BodyHandler<T> bodyHandler) {
        // 第一次尝试
        HttpResponse<T> response = requestManager.getSession().send(request, bodyHandler);

        // 如果是认证/授权失败
        if (response.statusCode() == 401 || response.statusCode() == 403) {
            log.warn("请求未授权({}), 可能 session 已失效，尝试重新登录...", response.statusCode());
            // 执行重新登录 (这里的url, user, pass需要能获取到)
            loginToJenkins(jenkins_local_base_url, jenkins_local_username, jenkins_local_base_password);
            log.info("重新登录成功，重试之前的请求...");
            // 重试一次
            response = requestManager.getSession().send(request, bodyHandler);
        }
